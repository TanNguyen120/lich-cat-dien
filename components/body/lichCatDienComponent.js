...
import { getScheduleFromCungCau, getScheduleFromIthongTin } from '../utility/webCrawling';
    ...
    useEffect(() => {
        ...
        const getCrawlingData = async () => {
            ...
            if (currentSrc !== null) {
                switch (currentSrc) {
                    case 'CungCau':
                        const result = await getScheduleFromCungCau();
                        setSchedule(result);
                        break;
                    case 'iThongTin':
                        const result2 = await getScheduleFromIthongTin();
                        setSchedule(result2)
                        break;
                    default:
                        break;
